...
import { Bot, Send, X, Trash2, Expand, Sun, Moon } from 'lucide-react';
...
export function ChatWidget() {
    ...
    const [suggestedQuestions, setSuggestedQuestions] = useState([]); // <-- new state
    const [theme, setTheme] = useState('light');
    const textareaRef = useRef(null);
    const messagesEndRef = useRef(null);
    const smallChatScrollRef = useRef(null);
    const expandedModalScrollRef = useRef(null);
    ...
    const sendMessageWithTopic = async (messageText, topicOverride) => {
        if (!messageText.trim())
            return;
        ...
        try {
            ...
            const botMessage = { id: crypto.randomUUID(), role: 'assistant', content: aiResponse };
    ...
    return (<div>
      ...
      {/* Expanded Modal View */}
      {isExpanded && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
           <div className="w-full max-w-5xl h-[80vh] flex flex-row rounded-2xl border-border bg-card overflow-hidden">
               ...
               <Card className="h-full flex flex-col shadow-none rounded-none border-0">
                ...
                <CardTitle className="text-lg font-semibold text-blue-900 dark:text-white">Alvin Helpdesk</CardTitle>
                  ...
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-blue-900 dark:text-white hover:bg-blue-100 dark:hover:bg-blue-900/40 group transition" onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}>
                      {theme === 'dark'
                ...
                : <Moon className="h-5 w-5 group-hover:text-blue-700 transition"/>}
                    ...
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-blue-900 dark:text-white hover:bg-gray-200 dark:hover:bg-gray-700/60 group transition" onClick={() => setIsExpanded(false)}>
                ...
                <CardContent className="flex-1 p-0 flex flex-col min-h-0">
                  <ScrollArea className="flex-1 h-full" ref={expandedModalScrollRef}>
                    <div className="space-y-6 p-6">
                      ...
                      {isLoading && (<div className="flex items-start gap-3 justify-start animate-in fade-in duration-300">
                      ...
                      {showTopics && !isLoading && (<div className="flex flex-col items-center justify-center min-h-[60vh] text-center text-muted-foreground">
                          ...
                          <div className="topic-cards grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1 sm:gap-1.5 md:gap-2 lg:gap-3 w-full max-w-3xl mx-auto px-1 sm:px-2 md:px-4">
                            {productionSuggestedTopics.map((category, index) => (<div key={category.category} className="group cursor-pointer" onClick={() => handleTopicSelect(category.category)}>
                      ...
                      {messages.length > 0 && suggestedQuestions.length > 0 && (<div className="flex flex-wrap gap-2 mt-2">
                          {suggestedQuestions.map((q, idx) => (<Button key={idx} variant="outline" size="sm" onClick={() => sendMessage(q)} className="text-xs hover:bg-blue-50 hover:border-blue-300 hover:text-blue-700 dark:hover:bg-blue-950 dark:hover:border-blue-600 dark:hover:text-blue-300">
                              {q}
                            </Button>))}
                        </div>)}
                ...
                <CardFooter className="p-4 border-t">
                  <form onSubmit={handleSubmit} className="flex w-full items-center gap-2">
                    <Textarea ref={textareaRef} name="message" placeholder="Type a message..." className="flex-1 resize-none overflow-y-auto min-h-[48px] max-h-[96px] py-2 px-3 text-sm rounded-md dark-scrollbar" value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={(e) => {
             ...
             <div className="w-1/3 text-white flex flex-col items-center justify-start p-8 relative overflow-hidden">
                <div className="absolute inset-0 flex items-center justify-center">
                    <Image src={alvinAvatar.src} alt="Alvin Avatar" fill className="object-cover" priority/>
                </div>
      ...
      <div className="fixed bottom-4 right-4 z-40 flex flex-col items-end gap-4">
        <div className={cn("transition-all duration-300 ease-in-out transform", isOpen && !isExpanded ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none")}>
          <Card className="w-[calc(100vw-32px)] sm:w-[380px] h-[60vh] sm:h-[550px] flex flex-col rounded-2xl border-border">
            ...
            : <Moon className="h-5 w-5 group-hover:text-blue-700 transition"/>}
                ...
                <Button variant="ghost" size="icon" className="h-8 w-8 text-white hover:bg-blue-700/80 transition" onClick={() => setIsExpanded(true)}>
                  <Expand className="h-5 w-5"/>
                </Button>
                <Button variant="ghost" size="icon" className="h-8 w-8 text-white hover:bg-red-600/80 transition" onClick={clearChat}>
                  <Trash2 className="h-5 w-5"/>
                </Button>
                <Button variant="ghost" size="icon" className="h-8 w-8 text-white hover:bg-gray-700/80 transition" onClick={() => setIsOpen(false)}>
                  <X className="h-5 w-5"/>
                </Button>
              </div>
            </CardHeader>
            <CardContent className="flex-1 p-0 flex flex-col min-h-0">
                {showTopics && !isLoading && (<div className="flex flex-col items-center justify-center min-h-[60vh] text-center bg-white dark:bg-[#181f2a] text-black dark:text-white px-4 pb-16 sm:pb-20 md:pb-24 lg:pb-28 xl:pb-32 xl-mid:pb-5 2xl:pb-48">
              ...
              <ScrollArea className="flex-1 h-full" ref={smallChatScrollRef}>
                <div className="space-y-6 p-4">
                  ...
                  {messages.length > 0 && suggestedQuestions.length > 0 && (<div className="flex flex-wrap justify-center gap-2 mt-2 px-2">
                      {suggestedQuestions.map((q, idx) => (<Button key={idx} variant="outline" size="sm" className="whitespace-normal break-words text-xs px-3 py-1 max-w-[70%]" onClick={() => sendMessage(q)}>
                          {q}
                        </Button>))}
                    </div>)}
                  <div ref={messagesEndRef}/>
        ...
        <button aria-label="Open Chat" className={cn("group relative flex items-center justify-end rounded-full w-16 h-16 transition-all duration-300 ease-in-out focus:outline-none", isOpen ? "hidden" : "block", "hover:w-[280px] hover:bg-muted")} onClick={() => setIsOpen(true)}>
